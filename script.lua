...
local library, pointers = loadstring(game:HttpGet("https://raw.githubusercontent.com/nozkel/revenant/refs/heads/main/lib.lua"))()
do


    local window = library:New({name = "Revenant" , size = Vector2.new(555, 610), Accent = Color3.fromRGB(192, 118, 227)})
    ...
    do
        ...
        do
            ...
            local function update_config_list()
                local list = {}
                for idx, file in ipairs(listfiles("Revenant/configs")) do
                    local file_name = file:gsub("Revenant/configs\\", ""):gsub(".txt", "")
            ...
            config_section:ButtonHolder({Buttons = {{"Create",  function()local config_name = pointers["settings/configuration/name"]:get()
                if config_name == "" or isfile("Revenant/configs/" .. config_name .. ".txt") then
                    return
                end

                writefile("Revenant/configs/" .. config_name .. ".txt", "")
                update_config_list() end}, {"Delete", function()
                local selected_config = pointers["settings/configuration/list"]:get()[1][1]
                if selected_config then
                    delfile("Revenant/configs/" .. selected_config .. ".txt")
                    update_config_list()
                end
                end}}})
            config_section:ButtonHolder({Buttons = {{"Load", function()
                local selected_config = pointers["settings/configuration/list"]:get()[1][1]
                if selected_config then
                    window:LoadConfig(readfile("Revenant/configs/" .. selected_config .. ".txt"))
                end
            end}, {"Save", function()
                local selected_config = pointers["settings/configuration/list"]:get()[1][1]
                if selected_config then
                    writefile("Revenant/configs/" .. selected_config .. ".txt", window:GetConfig())
                end
            end}}})
